...
export default function Index() {
  ...
  useEffect(() => {
    fetch("http://deliveryapi.praathee.in:3000/api/v1/order/getcustomer")
  ...
  const handleContinue = () => {
    if (selectedDate) {
      console.log(`Selected delivery date for ${selectedCustomer.fullname}: ${selectedDate}`);
      setShowModal(false);
      navigate("/productList");
    } else {
      alert("Please select a delivery date.");
  ...
  return (
    <Container fluid className="mt-4">
      ...
      <Row className="g-3">
        {filteredCustomers.length > 0 ? (
          filteredCustomers.map((customer, index) => (
            <Col xs={12} sm={6} md={4} key={index}>
              <UserCard customer={customer} onCardClick={() => handleOpenModal(customer)} navigate={navigate} />
            </Col>
          ))
        ) : (
          <Col xs={12}>
      ...
      <Modal show={showModal} onHide={handleCloseModal} centered>
        <Modal.Header closeButton>
          <Modal.Title>Select Delivery Date for {selectedCustomer?.fullname}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Form.Group>
            <Form.Label>Select a Date:</Form.Label>
...
// User Card Component
function UserCard({ customer, onCardClick, navigate }) {
  return (
    <motion.div
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
    ...
    >
      ...
      >
        ...
        >
          <motion.div whileHover={{ rotate: 10, scale: 1.2 }} transition={{ duration: 0.2 }} style={{ cursor: "pointer", color: "#229C47" }}>
            <FaEdit size={18} onClick={(e) => { e.stopPropagation(); navigate("/adduser-customer-form"); }} />
          </motion.div>
          <motion.div whileHover={{ rotate: 10, scale: 1.2 }} transition={{ duration: 0.2 }} style={{ cursor: "pointer", color: "#229C47" }}>
            <FaWallet size={18} onClick={(e) => { e.stopPropagation(); navigate("/wallet"); }} />
          </motion.div>
        </div>
        ...
        <div className="p-3" style={{ textAlign: "left", gap: "15px", display: "flex", flexDirection: "column", justifyContent: "center" }}>
          ...
          <div className="text-muted">
            <FaPhone className="me-1" />
            <span style={{ color: "#1E4FCB" }}>{customer.mobileno}</span>
          </div>
          <div className="text-muted">
            <CiLocationOn className="me-1" />
            <span>{customer.addressline1 || "No Address Available"}</span>
          </div>
        </div>
      </Card>
    </motion.div>
